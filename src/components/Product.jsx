    ...
    return (
        <div className="m-4 object-contain text-center">
            ...
            <div className="bg-pink-300 mb-6">
              ...
              <p>$ {prod.price}</p>
              {cart.includes(prod) ? (
                <button className="bg-red-400 m-2 w-72" onClick={() => {setCart(cart.filter((cartProd) => cartProd.id !== prod.id))}}>
                  Remove from cart
                </button>) 
                :
              <button className="bg-green-400 m-2 w-72" onClick={() => {setCart([...cart, prod])}}>
                  Add to cart
              </button>
              }
            </div>
